...
class BanquePopulaire(TwoFactorBrowser):
    ...
    def do_new_login(self):
        ...
        if oauth_token_client_id == "" or oauth_autorize_client_id == "":
            # Fallback: client IDs moved from JS chunks to xld-keys.json
        ...
        if oauth_token_client_id == "":
            self.logger.debug("No oauth_token_client_id found")
            raise BrowserUnavailable("No oauth_token_client_id found")

        if oauth_autorize_client_id == "":
            self.logger.debug("No oauth_autorize_client_id found")
            raise BrowserUnavailable("No oauth_autorize_client_id found")
