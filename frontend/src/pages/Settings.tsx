...
export function Settings() {
  ...
  async function handleConnectorSubmit(data: {
    ...
    try {
      ...
      const detail = (err as { detail?: string }).detail ?? 'Opération échouée';
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      const detail = (err as { detail?: string }).detail ?? 'Échec de la suppression du connecteur';
      setConnectorError(detail);
    }
  }
  ...
  async function handleChangePassword() {
    ...
    try {
      ...
      const detail = (err as { detail?: string }).detail ?? 'Échec de la modification du mot de passe';
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-medium text-mm-text">Connecteurs</h2>
          <button
            onClick={() => {
      ...
      {/* -- Vault Section ------------------------------------------------------ */}
      <div className="flex flex-col gap-4">
        <h2 className="text-lg font-medium text-mm-text">Coffre-fort</h2>
        ...
        <div className="bg-mm-surface border border-mm-border rounded-[12px] p-5 flex flex-col gap-5">
          ...
          <div className="border-t border-mm-border pt-5 flex flex-col gap-4">
            <h3 className="text-sm font-medium text-mm-text">Modifier le mot de passe</h3>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-mm-text-secondary">Mot de passe actuel</label>
              <input
                type="password"
                value={oldPassword}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-mm-text-secondary">Nouveau mot de passe</label>
              <input
                type="password"
                value={newPassword}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-mm-text-secondary">Confirmer le nouveau mot de passe</label>
              <input
                type="password"
      ...
      {/* -- System Section ----------------------------------------------------- */}
      <div className="flex flex-col gap-4">
        <h2 className="text-lg font-medium text-mm-text">Système</h2>
            ...
            {health && (
              <div className="bg-mm-surface border border-mm-border rounded-[12px] p-5 flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-medium text-mm-text">État du système</h3>
                </div>

                <div className="grid grid-cols-3 gap-4">
                  <div className="flex flex-col gap-1">
                    <span className="text-[11px] text-mm-text-muted">État</span>
                    <span className={`text-sm font-medium ${health.status === 'ok' ? 'text-green-400' : 'text-yellow-400'}`}>
                      {health.status}
                    </span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[11px] text-mm-text-muted">Coffre-fort</span>
                    <span className="text-sm font-medium text-mm-text">{health.vault}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[11px] text-mm-text-muted">Planificateur</span>
                    <span className={`text-sm font-medium ${health.scheduler === 'running' ? 'text-green-400' : 'text-gray-400'}`}>
                      {health.scheduler}
                    </span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[11px] text-mm-text-muted">Base de données</span>
                    <span className={`text-sm font-medium ${health.db === 'ok' ? 'text-green-400' : 'text-red-400'}`}>
                      {health.db}
                    </span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[11px] text-mm-text-muted">Temps de fonctionnement</span>
                ...
                {Object.keys(health.workers).length > 0 && (
                  <div className="border-t border-mm-border pt-3 flex flex-col gap-2">
                    <span className="text-[11px] text-mm-text-muted">Travailleurs</span>
            ...
            {scheduler && scheduler.jobs.length > 0 && (
              <div className="bg-mm-surface border border-mm-border rounded-[12px] p-5 flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-medium text-mm-text">Tâches planifiées</h3>
                </div>
                    ...
                    <div key={job.id} className="flex items-center gap-3 py-1.5">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-xs text-mm-text-muted">
                          Prochain : {formatRelativeTime(job.next_run)}
                        </span>
                        {job.last_result && (
