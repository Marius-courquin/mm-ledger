...
- **Persistance de session (connecteurs workers)** : à chaque connect réussi, le worker sérialise sa session (token TR, connexion IBKR, Woob storage) et le manager la stocke dans le vault SQLCipher (col `session` de la table `credentials`). Au restart, le vault est unlocked → le manager réinjecte la session dans le worker via `restore_session(blob)` → ping de validation → si OK, état `connected` direct, **pas de 2FA**. Si KO, fallback transparent sur le `connect(credentials)` complet. Spec : `docs/superpowers/specs/2026-04-29-session-persistence-design.md`. Méthodes overridables dans `ConnectorWorker.serialize_session()` / `restore_session()`. Note : **banking** (Enable Banking) n'a pas de ConnectorWorker — ses sessions sont stockées dans `data/users/{id}/banking_sessions.json` et ne passent pas par ce mécanisme. Note : **Woob** `restore_session` retourne False en v1 (la storage est sérialisée mais la restauration nécessite les credentials dans `load_backend`, qui arrivent séparément via `connect()`) — fallback sur connect() complet garanti.
